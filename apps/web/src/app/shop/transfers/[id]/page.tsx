...
export default function TransferDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <Link href="/shop" className="mb-4 inline-flex items-center gap-1 text-sm text-muted hover:text-ink">
        <ArrowRight className="size-4" /> رجوع إلى مخزن المحل
      </Link>
      ...
      <Table headers={["الصنف", "الكمية", "تكلفة الوحدة", "تكلفة الدفعة", "دفعة الرئيسي", "دفعة المحل"]}>
        {transfer.lines.map((line) => (
          <tr key={line.id}>
            ...
            <td className="px-4 py-3 tnum text-muted">#{line.shopBatchId}</td>
          </tr>
        ))}
      </Table>
