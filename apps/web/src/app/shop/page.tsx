...
import {
  ...
  ShopTabs,
  type ShopTab,
} from "@/components/transfers/shop-controls";
...
export default function ShopPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const [shopRows, requestRows, transferRows, mainRows] =
          await Promise.all([
            getShopWarehouseStock(),
            listTransferRequests(),
            listTransfers(),
        ...
        if (!cancelled)
          setError(
            caught instanceof Error
              ? caught.message
              : "تعذر تحميل مخزن المحل والتحويلات",
          );
      } finally {
    ...
    <div>
      <PageHeader
        ...
        actions={
          <ShopHeaderActions
            ...
            onRequest={() => setFormMode("request")}
            onDirect={() => setFormMode("direct")}
          />
        }
      />
      ...
      <section className="mb-6 overflow-hidden rounded-2xl border border-line bg-sidebar text-white shadow-[0_16px_45px_rgb(43_33_24/0.10)]">
        <div className="grid divide-y divide-white/10 sm:grid-cols-2 sm:divide-x sm:divide-x-reverse sm:divide-y-0 lg:grid-cols-5">
          <Summary
            icon={<Shirt className="size-5 text-accent" />}
            label="أصناف نشطة"
            value={String(activeStock.length)}
          />
          <Summary
      ...
      >
        {loading ? (
          <p className="text-muted">جارِ تحميل دفتر المحل…</p>
        ) : tab === "stock" ? (
  ...
  if (rows.length === 0)
    return (
      <Empty
        ...
        title="لا يوجد رصيد في المحل بعد"
      ...
      {rows.map((row) => (
        <tr key={row.itemId} className={row.isActive ? "" : "opacity-55"}>
          ...
          <td className="px-4 py-3">
            <Badge
              tone={
                row.isNegativeStock || row.isLowStock
                  ? "danger"
                  : row.isActive
                    ? "success"
                    : "neutral"
  ...
  if (rows.length === 0)
    return (
      <Empty
        ...
        description="طلبات فريق المحل ستظهر هنا للجميع لتجنب تكرار الاحتياج."
      />
    );
  return (
    <Table
      headers={["الطلب", "صاحب الطلب", "الأصناف", "وقت الطلب", "الحالة", ""]}
    >
      {rows.map((request) => (
        <tr key={request.id}>
          ...
          <td className="px-4 py-3">
            <button
              ...
              aria-label={`عرض طلب التحويل رقم ${request.id}`}
              title="عرض الطلب"
              className="rounded-lg p-2 text-muted hover:bg-line/50 hover:text-ink"
            >
    ...
    >
      {rows.map((transfer) => (
        <tr key={transfer.id}>
          ...
          <td className="px-4 py-3">
            <Link
              href={`/shop/transfers/${transfer.id}`}
...
function Empty({
  ...
  return (
    <div className="rounded-2xl border border-dashed border-line bg-surface p-10 text-center">
      <div className="mx-auto mb-3 w-fit text-muted">{icon}</div>
